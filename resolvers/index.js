...
const resolvers = {
    ...
    Mutation: {
        ...
        addBook: async (_, { title, genre, author, published_year }, context) => {
            // await auth(context);
        ...
        updateUser: async (_, { id, name, email, city, age, role }) => {
            const updatedUser = await UserModel.findByIdAndUpdate(id, { name, email, city, age, role }, { new: true });
            if (!updatedUser) {
                throw new Error("User not found");
            }
            return updatedUser;
        ...
        updateBook: async (_, { id, title, genre, author, published_year }) => {
            const updatedBook = await BookModel.findByIdAndUpdate(id, { title, genre, author, published_year }, { new: true });
            if (!updatedBook) {
                throw new Error("Book not found");
            }
            return updatedBook;
